...
export const CovidStore = {
    ...
    service: {
        getChartData: (arr: CovidDataType[]) => {
            const arrTotal: any = [];
            const dateQtyMap: any = {};
            ...
            return arrTotal.sort((a: any, b: any) => {
        ...
        filterAllData: (keyVal: string, from: number, to: number, startDateData: Date, endDateData: Date, country: string, sortConfig: string, field: string) => {
            const rawData = CovidStore.store.getState().data;

            const minCases = Math.min(...rawData.map((item: any) => item[keyVal]));
            const maxCases = Math.max(...rawData.map((item: any) => item[keyVal]));
            ...
            return CovidStore.service.filterChartData(country, startDateData, endDateData)
              .filter((item: any) => item[keyVal] >= valueFrom && item[keyVal] <= valueTo)
              .sort((a: any, b: any) => {
                  const comparison = sortConfig === 'ascend' ? 1 : -1;
                  return a[field] < b[field] ? -comparison : comparison;
              });
        },
        ...
        filterChartData: (country: string, startDateData: Date, endDateData: Date) => {
            ...
            rawData.forEach((el: any) => {
                const parsedDate = new Date(el.dateRep.split('/').reverse().join('-'));

                if (el.countriesAndTerritories.includes(country)) {
                    filteredArr.push({
                        ...el,
            ...
            return filteredArr
              .filter((item: any) => item.parsedDate >= parsedStartDate && item.parsedDate < parsedEndDate)
        },

        getInitialParam: (arr: any) => {
            const parseDate = (dateStr: any) => {
                return new Date(dateStr.split('/').reverse().join('-'));
            };
            const dates = arr.map((item: any) => parseDate(item.dateRep));
            ...
            CovidStore.store.setState({
                ...
                filterParam: {
                    ...
                    startDate: dateToSting(new Date(Math.min(...dates))),
                    endDate: dateToSting(new Date(Math.max(...dates))),
                },
        ...
        getCovidData: async () => {
            try {
                ...
                const newRecord = (records: CovidDataType[]) => {
                    const totalDeathsByCountry: any = {};
                ...
                if (response) {
                    await CovidStore.store.setState({
                        ...
                        country: response.data.records.reduce((acc: any, curr: any) => {
                            const existingItem = acc.find((item: any) => item.value === curr.countriesAndTerritories);
                            if (!existingItem) {
        ...
        filterData: (key: string, from: number, to: number) => {
            const state = CovidStore.store.getState().filterParam
            const sortArr = CovidStore.service.filterAllData(key, from, to, state.startDate, state.endDate, state.searchParam, state.sortConfig, state.field)
